...
After a preliminary analysis of the data, we can show the cumulative number of species discovered over time for Australia as a whole. Since the data in the `evenDate` column is all characters, we need to transform it to Date format before ploting.
...
data_sorted <- data_2 |>
  arrange(Date_Object) |>
  # The `cumsum` function can help calculate the cumulative number of species discovered so far. But it can't include previously duplicated species.
...
We have sorted the data by date and has a cumulative count of species discovered up to each date before. Next, the **ChatGwe can use the `nls()` function (nonlinear least square method) in R to fit the logistic growth curve to the data. Then use the fitted model to predict the total number of species.
...
plot_3 <- plot_3 +
  geom_hline(yintercept = 196, color = "purple", linetype = "dashed", linewidth = 0.75) +
  geom_hline(yintercept = 202, color = "darkgreen", linetype = "dashed", linewidth = 0.75) +
  geom_hline(yintercept = 246, color = "red", linetype = "dashed", linewidth = 0.75) +
